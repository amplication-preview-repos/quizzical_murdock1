/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { NormalCustomFieldWhereInput } from "./NormalCustomFieldWhereInput";
import { ValidateNested, IsOptional } from "class-validator";
import { Type } from "class-transformer";

@InputType()
class NormalCustomFieldListRelationFilter {
  @ApiProperty({
    required: false,
    type: () => NormalCustomFieldWhereInput,
  })
  @ValidateNested()
  @Type(() => NormalCustomFieldWhereInput)
  @IsOptional()
  @Field(() => NormalCustomFieldWhereInput, {
    nullable: true,
  })
  every?: NormalCustomFieldWhereInput;

  @ApiProperty({
    required: false,
    type: () => NormalCustomFieldWhereInput,
  })
  @ValidateNested()
  @Type(() => NormalCustomFieldWhereInput)
  @IsOptional()
  @Field(() => NormalCustomFieldWhereInput, {
    nullable: true,
  })
  some?: NormalCustomFieldWhereInput;

  @ApiProperty({
    required: false,
    type: () => NormalCustomFieldWhereInput,
  })
  @ValidateNested()
  @Type(() => NormalCustomFieldWhereInput)
  @IsOptional()
  @Field(() => NormalCustomFieldWhereInput, {
    nullable: true,
  })
  none?: NormalCustomFieldWhereInput;
}
export { NormalCustomFieldListRelationFilter as NormalCustomFieldListRelationFilter };
