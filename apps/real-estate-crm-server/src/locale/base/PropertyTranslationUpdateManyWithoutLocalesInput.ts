/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { PropertyTranslationWhereUniqueInput } from "../../propertyTranslation/base/PropertyTranslationWhereUniqueInput";
import { ApiProperty } from "@nestjs/swagger";

@InputType()
class PropertyTranslationUpdateManyWithoutLocalesInput {
  @Field(() => [PropertyTranslationWhereUniqueInput], {
    nullable: true,
  })
  @ApiProperty({
    required: false,
    type: () => [PropertyTranslationWhereUniqueInput],
  })
  connect?: Array<PropertyTranslationWhereUniqueInput>;

  @Field(() => [PropertyTranslationWhereUniqueInput], {
    nullable: true,
  })
  @ApiProperty({
    required: false,
    type: () => [PropertyTranslationWhereUniqueInput],
  })
  disconnect?: Array<PropertyTranslationWhereUniqueInput>;

  @Field(() => [PropertyTranslationWhereUniqueInput], {
    nullable: true,
  })
  @ApiProperty({
    required: false,
    type: () => [PropertyTranslationWhereUniqueInput],
  })
  set?: Array<PropertyTranslationWhereUniqueInput>;
}

export { PropertyTranslationUpdateManyWithoutLocalesInput as PropertyTranslationUpdateManyWithoutLocalesInput };
